...
use tracing::{field::FieldSet, info, instrument, level_filters::STATIC_MAX_LEVEL, Level, Metadata};
use tracing_core::{callsite::DefaultCallsite, identify_callsite, Callsite, LevelFilter};

use crate::TuiContext;

#[instrument(skip_all)]
async fn init_mpv(cx: &TuiContext, item: MediaItem) -> Result<(Vec<MediaItem>,Mpv<EventContextAsync>)> {
...
#[instrument(skip_all)]
pub async fn play(cx: &TuiContext, item: MediaItem)->Result<()>{
    let (items, mpv) = init_mpv(cx, item).await?;

    info!("time-pos: {}",mpv.get_property::<i64>("time-pos")?);
...
async fn recv_mpv_events(mpv: &mut Mpv<EventContextAsync>) -> Result<bool> {
    loop {
        tokio::select! {
            event = mpv
                .wait_event_async() => {
        ...
        mpv.get_property_async::<i64>("time-pos", 6969)?;
    }
}
